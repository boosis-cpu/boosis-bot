...
const VisionChart = ({ initialSymbol, symbol: syncedSymbol, timeframe: syncedTimeframe, token, onPattern, mode = 'price', priceType = 'candle', showIndicators = false, onSymbolChange, onTimeframeChange }) => {
    ...
    const [timeframe, setTimeframe] = useState(syncedTimeframe || localStorage.getItem('vision_tf') || '15m');
    ...
    const calculateMACD = (data, fast, slow, signal) => {
        ...
        const macdLine = fastEma.map((f, i) => ({ time: f.time, value: f.value - slowEma[i].value }));
        const signalLine = calculateEMA(macdLine, signal);
        const hist = macdLine.map((m, i) => {
            const s = signalLine[i];
            const val = m.value - s.value;
            return { time: m.time, value: val, color: val >= 0 ? 'rgba(38, 166, 154, 0.5)' : 'rgba(239, 83, 80, 0.5)' };
        });
    ...
    const applyTimeShift = (candles) => {
        ...
        return candles.map(c => {
            let base;
            if (Array.isArray(c)) {
                base = { time: Math.floor(Number(c[0]) / 1000), open: parseFloat(c[1]), high: parseFloat(c[2]), low: parseFloat(c[3]), close: parseFloat(c[4]), volume: parseFloat(c[5]) };
            } else {
    ...
    useEffect(() => { if (syncedSymbol) setSymbol(syncedSymbol); }, [syncedSymbol]);
    useEffect(() => { if (syncedTimeframe) setTimeframe(syncedTimeframe); }, [syncedTimeframe]);
    ...
    useEffect(() => {
        ...
        let mainSeries;
        if (volumeOnly) {
            mainSeries = chart.addHistogramSeries({ color: '#26a69a', priceScaleId: '', priceFormat: { type: 'volume' } });
            mavol1Ref.current = chart.addLineSeries({ color: '#00bcd4', lineWidth: 1.5, priceLineVisible: false, lastValueVisible: false });
            mavol2Ref.current = chart.addLineSeries({ color: '#e91e63', lineWidth: 1.5, priceLineVisible: false, lastValueVisible: false });
        } else if (macdOnly) {
            mainSeries = chart.addLineSeries({ lineWidth: 0, priceLineVisible: false, lastValueVisible: false });
            macdHistRef.current = chart.addHistogramSeries({ color: '#26a69a', priceFormat: { type: 'price', precision: 4, minMove: 0.0001 }, lastValueVisible: true });
            macdLineRef.current = chart.addLineSeries({ color: '#bb86fc', lineWidth: 1.5, priceFormat: { type: 'price', precision: 4, minMove: 0.0001 }, priceLineVisible: false, lastValueVisible: true });
            macdSignalRef.current = chart.addLineSeries({ color: '#ff4081', lineWidth: 1.5, priceFormat: { type: 'price', precision: 4, minMove: 0.0001 }, priceLineVisible: false, lastValueVisible: true });
            chart.priceScale('right').applyOptions({ scaleMargins: { top: 0.1, bottom: 0.1 }, visible: true, borderVisible: true });
        } else {
            if (priceType === 'line') {
                mainSeries = chart.addLineSeries({ color: '#26a69a', lineWidth: 2, priceFormat: { type: 'price', precision: 6, minMove: 0.000001 } });
            } else {
                mainSeries = chart.addCandlestickSeries({ upColor: '#26a69a', downColor: '#ef5350', borderVisible: false, wickUpColor: '#26a69a', wickDownColor: '#ef5350', priceFormat: { type: 'price', precision: 6, minMove: 0.000001 } });
            }
            if (showIndicators) {
            ...
            try {
                ...
                if (res && res.data && res.data.candles) {
                    ...
                    if (isInitial) {
                        ...
                        if (volumeOnly) {
                            const volData = newCandles.map(c => ({ time: c.time, value: c.volume, color: c.close >= c.open ? 'rgba(38, 166, 154, 0.5)' : 'rgba(239, 83, 80, 0.5)' }));
                        ...
                        // Zoom inicial estilo Binance: mostrar solo las últimas N velas según timeframe
                        const visibleBars = { '1m': 180, '5m': 150, '15m': 120, '30m': 100, '1h': 80, '4h': 60, '1d': 60 };
                        const barsToShow = visibleBars[timeframe] || 100;
                        const totalBars = newCandles.length;
                        ...
                        if (volumeOnly) {
                            mainSeries.update({ time: lastCandle.time, value: lastCandle.volume, color: lastCandle.close >= lastCandle.open ? 'rgba(38, 166, 154, 0.5)' : 'rgba(239, 83, 80, 0.5)' });
                        } else if (macdOnly) {
                            const { macd, signal, hist } = calculateMACD(newCandles, macdFast, macdSlow, macdSignal);
                            if (macd.length > 0) {
                                ...
                                setMacdValues({ macd: macd[macd.length - 1].value, signal: signal[signal.length - 1].value, hist: hist[hist.length - 1].value });
                            }
                        } else {
                            if (priceType === 'line') {
        ...
        const drawPattern = (pattern) => {
            ...
            if (patternSeriesRef.current) { try { chart.removeSeries(patternSeriesRef.current); } catch (e) { } }
            if (necklineRef.current) { try { seriesRef.current.removePriceLine(necklineRef.current); } catch (e) { } }
            const formattedPoints = pattern.drawingPoints.map(pt => ({ time: applyTimeShiftToPoint(pt.time), value: pt.price }));
            const colors = { BULLISH: '#00ff88', BEARISH: '#ff4d4d' };
            const pSeries = chart.addLineSeries({ color: colors[pattern.direction] || '#f39c12', lineWidth: 3, lastValueVisible: false, priceLineVisible: false });
            pSeries.setData(formattedPoints);
            patternSeriesRef.current = pSeries;
            const markers = pattern.drawingPoints.map(pt => ({ time: applyTimeShiftToPoint(pt.time), position: pt.price > (pattern.neckline || pt.price) ? 'aboveBar' : 'belowBar', color: '#ffffff', shape: 'circle', text: pt.label }));
            pSeries.setMarkers(markers);
            if (pattern.neckline) {
                necklineRef.current = seriesRef.current.addPriceLine({ price: pattern.neckline, color: '#f39c12', lineWidth: 2, lineStyle: 1, title: 'NECKLINE', axisLabelVisible: true });
            }
        };
        ...
        socket.onmessage = (event) => {
            ...
            if (msg.time && msg.symbol === symbol) {
                ...
                // Calculamos el bucket en UTC y LUEGO aplicamos el shift para que coincida con setData
                const browserOffsetSecs = new Date().getTimezoneOffset() * 60;
                const shiftSecs = -browserOffsetSecs;
                const originalUtcTime = Number(msg.time);
                const bucketTimeUtc = Math.floor(originalUtcTime / intervalSecs) * intervalSecs;
                const bucketTime = bucketTimeUtc + shiftSecs;

                const shiftedMsg = { ...msg, time: bucketTime };

                const existingIdx = lastDataRef.current.findIndex(d => d.time === bucketTime);
                if (existingIdx !== -1) {
                    const current = lastDataRef.current[existingIdx];
                    lastDataRef.current[existingIdx] = { ...current, high: Math.max(current.high || current.value, msg.high || msg.close), low: Math.min(current.low || current.value, msg.low || msg.close), close: msg.close, volume: (current.volume || 0) + (msg.volume || 0) };
                ...
                if (isMounted && mainSeries && updatedCandle) {
                    if (volumeOnly) {
                        mainSeries.update({ time: bucketTime, value: updatedCandle.volume, color: updatedCandle.close >= updatedCandle.open ? 'rgba(38, 166, 154, 0.5)' : 'rgba(239, 83, 80, 0.5)' });
                    } else if (macdOnly) {
                        const { macd, signal, hist } = calculateMACD(lastDataRef.current, macdFast, macdSlow, macdSignal);
                        if (macd.length > 0) {
                            macdLineRef.current?.update(macd[macd.length - 1]);
                        ...
                        if (priceType === 'line') {
                            ...
                            mainSeries.update({ time: bucketTime, open: updatedCandle.open, high: updatedCandle.high, low: updatedCandle.low, close: updatedCandle.close });
                        }
                        if (showIndicators) {
    ...
    const handleSaveSettings = () => {
        if (volumeOnly) { localStorage.setItem('boosis_mavol1', mav1Period); localStorage.setItem('boosis_mavol2', mav2Period); }
        else if (macdOnly) { localStorage.setItem('boosis_macd_fast', macdFast); localStorage.setItem('boosis_macd_slow', macdSlow); localStorage.setItem('boosis_macd_signal', macdSignal); }
        else { localStorage.setItem('boosis_ma1', ma1Period); localStorage.setItem('boosis_ma2', ma2Period); localStorage.setItem('boosis_ma3', ma3Period); localStorage.setItem('boosis_boll_period', bollPeriod); localStorage.setItem('boosis_boll_std', bollStd); }
        setShowSettings(false);
    };

    const handleResetSettings = () => {
        if (volumeOnly) { setMav1Period(7); setMav2Period(14); }
        else if (macdOnly) { setMacdFast(12); setMacdSlow(26); setMacdSignal(9); }
        else { setMa1Period(7); setMa2Period(25); setMa3Period(99); setBollPeriod(20); setBollStd(2); }
    };
    ...
    return (
        <div className={`vision-chart-card ${mode !== 'price' ? 'special-mode' : ''}`}>
            <div className="card-header binance-style">
                <div className="left-controls" ref={settingsRef} style={{ flex: 1 }}>
                    {(!volumeOnly && !macdOnly && onSymbolChange) ? (
                        <div className="compact-header-container">
                            <div className="header-top-row">
                                <select value={symbol} onChange={(e) => handleSymbolChange(e.target.value)} className="symbol-selector">
                                    {availablePairs.map(p => <option key={p} value={p}>{p}</option>)}
                                </select>
                                <div className="timeframe-selector">
                                    {['1m', '5m', '15m', '1h', '4h', '1d'].map(tf => (
                                        <button key={tf} className={`tf-btn ${timeframe === tf ? 'active' : ''}`} onClick={() => handleTimeframeChange(tf)}>{tf}</button>
                                    ))}
                                </div>
                                {showIndicators && (
                            ...
                            {showSettings && (
                                <div className="volume-settings-popover">
                                    <div className="settings-fields">
                                        <div className="setting-item"><label>MA 1:</label><input type="number" value={ma1Period} onChange={(e) => setMa1Period(Number(e.target.value))} min="2" max="200" /></div>
                                        <div className="setting-item"><label>MA 2:</label><input type="number" value={ma2Period} onChange={(e) => setMa2Period(Number(e.target.value))} min="2" max="200" /></div>
                                        <div className="setting-item"><label>MA 3:</label><input type="number" value={ma3Period} onChange={(e) => setMa3Period(Number(e.target.value))} min="2" max="200" /></div>
                                        <div className="setting-item"><label>BOLL Período:</label><input type="number" value={bollPeriod} onChange={(e) => setBollPeriod(Number(e.target.value))} min="5" max="100" /></div>
                                        <div className="setting-item"><label>BOLL Desv.:</label><input type="number" value={bollStd} onChange={(e) => setBollStd(Number(e.target.value))} min="1" max="5" /></div>
                                    </div>
                            ...
                            {showSettings && (
                                <div className="volume-settings-popover">
                                    <div className="settings-fields">
                                        {volumeOnly ? (
                                            <>
                                                <div className="setting-item"><label>MAVOL 1:</label><input type="number" value={mav1Period} onChange={(e) => setMav1Period(Number(e.target.value))} min="2" max="50" /></div>
                                                <div className="setting-item"><label>MAVOL 2:</label><input type="number" value={mav2Period} onChange={(e) => setMav2Period(Number(e.target.value))} min="2" max="50" /></div>
                                            </>
                                        ) : macdOnly ? (
                                            <>
                                                <div className="setting-item"><label>Rápida:</label><input type="number" value={macdFast} onChange={(e) => setMacdFast(Number(e.target.value))} min="2" max="50" /></div>
                                                <div className="setting-item"><label>Lenta:</label><input type="number" value={macdSlow} onChange={(e) => setMacdSlow(Number(e.target.value))} min="5" max="100" /></div>
                                                <div className="setting-item"><label>Señal:</label><input type="number" value={macdSignal} onChange={(e) => setMacdSignal(Number(e.target.value))} min="2" max="50" /></div>
                                            </>
                                        ) : (
                                            <>
                                                <div className="setting-item"><label>MA 1:</label><input type="number" value={ma1Period} onChange={(e) => setMa1Period(Number(e.target.value))} min="2" max="200" /></div>
...
const PatternVision = ({ token }) => {
    ...
    const [focusTimeframe, setFocusTimeframe] = useState(() => localStorage.getItem('vision_tf') || '15m');
    const [isFullscreen, setIsFullscreen] = useState(false);
    const containerRef = useRef();

    useEffect(() => { localStorage.setItem('vision_symbol', focusSymbol); }, [focusSymbol]);
    useEffect(() => { localStorage.setItem('vision_tf', focusTimeframe); }, [focusTimeframe]);

    const handleNewPattern = (msg) => {
        setDetections(prev => ({ ...prev, [msg.symbol]: msg }));
    };

    // Sincronizar estado con el evento fullscreenchange del browser (ESC nativo)
    useEffect(() => {
    ...
    return (
        ...
        >
            ...
            <button
                ...
                title={isFullscreen ? 'Salir de pantalla completa (ESC)' : 'Pantalla completa — 4 paneles'}
                style={{
                    position: 'absolute',
                    top: '8px',
                    right: '12px',
